...
#args <- c("LWL8DEV", "289", "TEST_INT"); setup <- try(r_session_setup(args, test_mode = TRUE))
...
if (inherits(e, 'try-error')) {
  write_db_log(paste("Probleem bij aanmaken plaatdata: ", e ), "E")
  stop(e)
} else {
  if (nrow(dfPlates) == 0) {
    write_db_log(paste("Plaatdata: geen records gevonden"), "E")
    stop("Geen data gevonden")
  } else {
    write_db_log(paste("Plaatdata aangemaakt:", nrow(dfPlates), "records" ), "P")
  }
}
