...
#' Copy package scripts and resources to a single LIMS path if they are outdated.
#'
...
#' existing scripts and copies the new ones over. It also copies the 'resources'
#' folder and its contents to the target path.
#'
...
copy_and_update_scripts <- function(target_path) {
  ...
  # Get package script files (excluding files starting with underscore)
  ...
  # --- 5. Copy Resources Folder ---
  # Check if resources folder exists in the package
  package_resources_dir <- file.path(package_script_dir, "resources")
  
  if (dir.exists(package_resources_dir)) {
    message("    -> Copying 'resources' folder and its contents...")
    
    # Define target resources directory
    target_resources_dir <- file.path(target_path, "resources")
    
    # Backup existing resources folder if it exists
    if (dir.exists(target_resources_dir)) {
      timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
      backup_resources_path <- file.path(target_path, paste0("_backup_resources_", timestamp))
      
      backup_resources_result <- try({
        dir.create(backup_resources_path, showWarnings = FALSE, recursive = TRUE)
        file.copy(from = target_resources_dir, to = dirname(backup_resources_path), 
                  recursive = TRUE, overwrite = FALSE)
        file.rename(from = file.path(dirname(backup_resources_path), "resources"),
                    to = backup_resources_path)
      }, silent = TRUE)
      
      if (inherits(backup_resources_result, "try-error")) {
        warning(" -> Failed to backup existing resources folder.")
      } else {
        message(paste("    -> Backed up existing resources to:", backup_resources_path))
      }
      
      # Remove old resources folder to avoid conflicts
      unlink(target_resources_dir, recursive = TRUE)
    }
    
    # Create target resources directory
    dir.create(target_resources_dir, showWarnings = FALSE, recursive = TRUE)
    
    # Copy all files from package resources to target resources
    resources_files <- list.files(package_resources_dir, full.names = TRUE, recursive = TRUE)
    
    if (length(resources_files) > 0) {
      # Get relative paths to maintain folder structure
      relative_paths <- sub(paste0(package_resources_dir, .Platform$file.sep), "", resources_files)
      
      # Create subdirectories if needed
      subdirs <- unique(dirname(relative_paths))
      subdirs <- subdirs[subdirs != "."]
      if (length(subdirs) > 0) {
        for (subdir in subdirs) {
          dir.create(file.path(target_resources_dir, subdir), 
                     showWarnings = FALSE, recursive = TRUE)
        }
      }
      
      # Copy each file to maintain structure
      copy_resources_success <- try({
        result <- mapply(
          function(from, to) {
            file.copy(from = from, 
                      to = file.path(target_resources_dir, to), 
                      overwrite = TRUE)
          },
          resources_files,
          relative_paths
      ...
      if (inherits(copy_resources_success, "try-error") || !copy_resources_success) {
        warning(" -> Failed to copy some resources files.")
        return(FALSE)
      }
      
      message(paste("    -> Successfully copied", length(resources_files), "resource file(s)."))
    } else {
      message("    -> Resources folder is empty, nothing to copy.")
    }
  } else {
    message("    -> No 'resources' folder found in package, skipping resource copy.")
  }
