...
#' @param add_nominal logical indicating if a theoretical value should also be plotted (in the column with name nominal_value)
#' @param ... not currently in use
#' @importFrom ggplot2 ggplot geom_line geom_point aes xlab ylab
#' @importFrom rlang .data
#' @return Shewhart ggplot
#' @export
gg_lims_shewhart <- function(plotdata, red_rules = 1:2, orange_rules = 3:6, yellow_rules = 7:8, add_nominal = TRUE, ...){
  ...
  g <-
    ...
    if (!is.null(plotdata$nominal_value) & add_nominal == TRUE) {
      g <- g + geom_line(aes(y = .data$nominal_value), lty = 3, color = "green3")
    }
  g <- g +
    geom_line(aes(y = .data$center), lty = 2, color = "darkblue") +
    geom_line(lty = 3) +
