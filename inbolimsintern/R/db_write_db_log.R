...
#' }
#' @export
write_db_log <- function(message, status, call_id = NULL, user_name = NULL, conn = NULL, extra = "") {
  # Environment lookup
  if (is.null(call_id)) call_id <- get("call_id", envir = parent.frame())
  if (is.null(user_name)) user_name <- get("user_name", envir = parent.frame())
  if (is.null(conn)) conn <- get("conn", envir = parent.frame())
  
  ts <- Sys.time()
  
  tryCatch({
    sql <- "INSERT INTO C_RSCRIPT_LOG (call_id, status, timestamp, log_message, lims_user) VALUES (?, ?, ?, ?, ?);"
    invisible(DBI::dbExecute(conn, sql, params = list(call_id, status, ts, message, user_name)))
  }, error = function(e) {
    # Fallback to local log if DB is locked/down
    try(cat(as.character(ts), "!!! DB LOG FAIL !!!", message, as.character(e), "\n", 
        file = "D:/LABO_FS_PRD/_PIETER/rsession_critical_failures.log", append = TRUE))
  })
}
